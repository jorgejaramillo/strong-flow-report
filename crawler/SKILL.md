...
Luego pregunta (o infiere del contexto de la conversación) **para qué cliente**
es (el `slug` de `clients/<slug>/config.json`). El output siempre se guarda en
la carpeta del reporte del día de ese cliente:

```
reports/<slug>/<YYYY-MM-DD>/data/crawl/
```

usando la fecha de hoy. Si esa carpeta no existe todavía, créala.

---

## Opción A — 1 URL

1. Pide la URL y el cliente
2. Usa el endpoint `/content` de Cloudflare Browser Rendering para obtener el HTML renderizado
3. Guarda el resultado como archivo `.html` en `reports/<slug>/<YYYY-MM-DD>/data/crawl/`
...
1. Pide la URL de inicio (o patrón de URLs) y el cliente
2. Pregunta cuántas páginas como máximo (default: 10, o usa `crawl.maxPages` del `config.json` del cliente si existe)
...
Guarda cada página como `{slug}.html` en `reports/<slug>/<YYYY-MM-DD>/data/crawl/`.
