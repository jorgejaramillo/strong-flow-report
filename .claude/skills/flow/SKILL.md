...
| 6 | `landingsCrawl` | `node scripts/sections/landings-crawl.js <slug> <reportDate>` — requiere `CF_ACCOUNT_ID`/`CF_API_TOKEN` en `.env`. Si la descarga rápida trae menos de 200 caracteres de texto visible (sitios con hidratación JS lenta), reintenta esa URL con `gotoOptions.waitUntil: networkidle0` + `waitForTimeout: 5000ms` — un solo reintento, no siempre. Cada item de `landingsCrawl` queda con `retried: boolean`. |
...
| 10 | `sitemap` | `node scripts/sections/sitemap.js <slug> <reportDate>` — trae la lista de sitemaps registrados en GSC (`list_sitemaps`), descarga el XML real de cada uno (siguiendo un nivel de sitemap index si aplica) guardándolo en `reports/<slug>/<reportDate>/data/sitemaps/`, y con las URLs extraídas arma `seedUrlsNotInSitemap`: qué `crawl.seedUrls` no aparecen en ningún sitemap descargado. La indexación real (`seedUrlsIndexing`) sigue viniendo de `inspect_url` por separado — el campo `indexed` de `list_sitemaps` no es confiable. |
